...
garch.roll = function(Data=Datos[,serie], model="gjrGARCH", garchOrder=c(1,1), armaOrder=NULL, 
  ...
  for (i in 1:total.rolling){
    ...
    if (i==1 | i%%refit.every==0){ #Permite reestimar el modelo cada <refit.every> ventanas
      ...
      coef.test           = try(coef(model.fit)) # Se define la prueba para coeficientes inexistentes puesto que no se puede incluir en la prueba lógica. 
      if(class(model.fit)!='uGARCHfit')error.estim=1 
      else {
        if(class(coef.test)=="try-error")error.estim=1
        else{
          if (armaOrder[1]>0){ 
            Mod.roots=try(Mod(roots(c(1,(-coef(model.fit)[2:(armaOrder[1]+1)])))))
            #Modelo no estacionario
            if (class(Mod.roots)!="try-error") {
