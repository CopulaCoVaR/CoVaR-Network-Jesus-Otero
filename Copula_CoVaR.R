...
CoVaR.Copula <- function(Serie.1='CDS_Brazil', Serie.2='WTI', Datos=G20_Crisis_WNA, ALPHA=0.05,
  ...
  if(forecast.type == 'in sample'){
    model.Serie.1     = ugarchspec(variance.model=list(model="gjrGARCH", garchOrder=c(1,1)), 
                                   mean.model=list(armaOrder=ARMA.Order[Serie.1,], external.regressors=external.regressors), distribution.model="sstd")
    ...
    if (ARMA.Order[1]==0 &  ARMA.Order[2]==0) {
      ...
      forecast.Serie.1.sigma =  as.matrix(model.Serie.1.fit@fit$sigma) 
    } else {
      forecast.Serie.1.mean  =  Datos[,Serie.1] - model.Serie.1.fit@fit$sigma
      forecast.Serie.1.sigma =  as.matrix(model.Serie.1.fit@fit$sigma)  
    }
    # -------
  ...
  if (forecast.type=='in sample') {
    sd_errors1 = model.Serie.1.fit@fit$residuals/model.Serie.1.fit@fit$sigma
  ...
  VaR             = as.xts(CoVaR.Cop$VaR)
  VaRUp           = as.xts(CoVaR.Cop$VaR.up)
  ...
  if (forecast.type=='in sample') 
     return(list(CoVaR=CoVaRDown, VaR=VaR, CoVaRUp=CoVaRUp, VaRUp=VaRUp, DeltaCoVaR=DeltaCoVaRDown, 
                DeltaCoVaRUp=DeltaCoVaRUp, CoVaRMedian=CoVaRMedian, garch.serie.1=model.Serie.1.fit,
                garch.serie.2=model.Serie.2.fit, Par1=par.Cop.1, Par2=par.Cop.2))
  if (forecast.type=='rolling')
     return(list(CoVaR=CoVaRDown, VaR=VaR, CoVaRUp=CoVaRUp, VaRUp=VaRUp, DeltaCoVaR=DeltaCoVaRDown, 
                  DeltaCoVaRUp=DeltaCoVaRUp, CoVaRMedian=CoVaRMedian, garch.serie.2=forecast.Serie.2, Par1=par.Cop.1, Par2=par.Cop.2))
}
...
CoVaR_DF = function(Data, Serie.1=Name.Series1, Serie.2='WTI', copulas, alpha, beta,
  ...
  if (forecast.type=='rolling') CoVaR.series=xts(matrix(0, nrow=nrow(Data[(window.size+1):nrow(Data),]) , 
                                                        ncol=length(Serie.1)), order.by=ichimoku::index(Data[(window.size+1):nrow(Data),])) 
  if (forecast.type=='in sample') CoVaR.series=xts(matrix(0, nrow=nrow(Data) , 
  ...
  for (i in Serie.1){
    ...
    if(i==Serie.1[1]){ 
      CoVaR.1=CoVaR.Copula(Serie.1=i, Serie.2=Serie.2, Datos=Data, ALPHA=alpha,
                           ...
                           external.regressors=external.regressors[,,i],
    ...
    if (plot==TRUE) {
      ...
      print(plot.xts(CoVaR.series[,i],type="l",col="red", ylim=c(min(CoVaR.series[,i]), max(CoVaRUp.series[,i])),xlab="Time",
