...
Rolling_GFEVD = function(Data, structure='BasicEN', window.size, BigVAR.window=NULL,
  ...
  alpha.seq          = seq(1, New_length, length.out=(Num.alphas.rolling+1))
  ...
  for (i in 1:New_length){
    ...
    if (i %in% alpha.seq) { #Obj en la cual se re-estima <alpha>
      BigVaR.spec   = Optimal.model(Data=DF, alpha=seq(0,1,0.05), auto.lambdas=T, auto.length=20, plot=FALSE, window.size=BigVAR.window)
      BigVAR        = BigVaR.spec$Model 
      alpha.actual  = BigVAR@alpha
    }else{ #Se usa el <alpha> estimado en las lineas anteriores
      BigVaR.spec   = Optimal.model(Data=DF, alpha=alpha.actual, auto.lambdas=T, auto.length=20, plot=FALSE, window.size=BigVAR.window)
      BigVAR        = BigVaR.spec$Model 
    }
    # Calculo de GIRF
  ...
  print(plot.xts(Total, main='Total dynamic connectivity', xlab='Time',ylab='%',type='l', col='steelblue', lwd=2)) # Se grafica la conectividad total
  ...
  for (kk in colnames(Data)){
      ...
      par(mfrow=c(3,1))
      print(plot.xts(From_Degree[, kk], main=paste0('Total directional connectedness from others to ',kk), xlab='Time',ylab='Proportion',type='l', col='steelblue', lwd=2))
      print(plot.xts(To_Degree[, kk],   main=paste0('Total directional connectedness to others from ', kk), xlab='Time',ylab='Proportion',type='l', col='steelblue', lwd=2))
      print(plot.xts(Net_Dynamic[, kk], main=paste0('Net total directional connectedness for ',kk), xlab='Time',ylab='Proportion',type='l', col='steelblue', lwd=2))
      print(abline(h = 0, col='tomato', lty = 'dotted', lwd = 2))
      print(mtext(paste('Dynamic connectivity of', kk), side=3, cex =1.3, line = 0.5, outer=TRUE))
  }
