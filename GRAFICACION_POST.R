...
for (kk in country) {
  ...
  if (0) {
    ...
    write.csv(Network.plot.data$table, file=paste0('Conect_Table_',kk,'.csv'))
  ...
  if(0)  {
    ...
    for (i in colnames){
      ...
      yaxis.left=TRUE, yaxis.right=TRUE, lty='solid', cex=1, cex.axis=0.8)) 
  ...
  if(0)  {
    ...
    # Preparación
      #Serie.1 = colnames(Dynamic_Network$From.Degree.Dynamic[,i])
      # Datos
      data.xts            = Dynamic_Network$Total.Dynamic #*100 
      data.plot           = fortify(data.xts)
      colnames(data.plot) = c("Time","Total")
      data.plot[,"Time"]  = as.Date(data.plot[,"Time"])
      
      # Graficación
      if(theme=="classic"){
        total.plot   = ggplot(data=data.plot, mapping=aes(x=Time, y=Total))+geom_line(colour="steelblue")+theme_classic() + labs(x = element_blank(), y = "Total")  +scale_x_date(expand=c(0,0), limits=c(data.plot[1,"Time"],data.plot[nrow(data.plot),"Time"]))
        #from.plot = ggplot(data=data.plot, mapping=aes(x=Time, y=From))  +geom_line(colour="steelblue")+theme_classic() + labs(x = element_blank(), y = "From")+scale_x_date(expand=c(0,0), limits=c(data.plot[1,"Time"],data.plot[nrow(data.plot),"Time"]))
        #net.plot  = ggplot(data=data.plot, mapping=aes(x=Time, y=Net))   +geom_line(colour="steelblue")+theme_classic() + labs(x = element_blank(), y = "Net") +scale_x_date(expand=c(0,0), limits=c(data.plot[1,"Time"],data.plot[nrow(data.plot),"Time"]))
        plot      =  total.plot
        pdf(file = paste0(Resultados,'/Graficas_CoVaR','_',kk,"_Total.pdf"), onefile=FALSE)
        print(plot + plot_annotation(title = paste("Total dynamic connectedness"),
                                     subtitle = paste0("Stressed series: ",kk)))     }    
      if(theme=="grey")   {
        total.plot=ggplot(data=data.plot, mapping=aes(x=Time, y=Total))+geom_line(colour="steelblue")+theme_gray() + labs(x = element_blank(), y = "Total")  +scale_x_date(expand=c(0,0), limits=c(data.plot[1,"Time"],data.plot[nrow(data.plot),"Time"])+2)
        #from.plot = ggplot(data=data.plot, mapping=aes(x=Time, y=From))  +geom_line(colour="steelblue")+theme_gray() + labs(x = element_blank(), y = "From")+scale_x_date(expand=c(0,0), limits=c(data.plot[1,"Time"],data.plot[nrow(data.plot),"Time"])+2)
        #net.plot  = ggplot(data=data.plot, mapping=aes(x=Time, y=Net))   +geom_line(colour="steelblue")+theme_gray() + labs(x = element_blank(), y = "Net") +scale_x_date(expand=c(0,0), limits=c(data.plot[1,"Time"],data.plot[nrow(data.plot),"Time"])+2)
        plot      =total.plot
  ...
  if(0){
    ...
    for (j in 1:ncol(x)) {
      if (test=='created.function') {
        Bootstrap.KS               = try(KS.bootstrap(x=as.matrix(x[,j]), y=as.matrix(y[,j]), 
                                                      n.boot=n.rep, density.plot=F,  alternative=alt))
        P.values[j,'P-value']       = Bootstrap.KS$p.value
        P.values[j,'Statistic']     = Bootstrap.KS$KS.1
      }else{
        Bootstrap.KS                = ksboot(x=as.matrix(x[,j]), y=as.matrix(y[,j]), alternative=alt)
        P.values[j,'P-value']       = Bootstrap.KS$ksboot.pvalue 
        KS.1                        = ks.test(x=as.matrix(x[,j]),y=as.matrix(y[,j]), alternative=alt)
        P.values[j,'Statistic']     = KS.1$statistic
      }
      P.values[j,'CoVaR average'] = mean(x[,j])
      P.values[j,'VaR average']   = mean(y[,j])
      P.values[j,'SD-VaR']        = sd(y[,j])
      P.values[j,'SD-CoVaR']      = sd(x[,j])
    }
    print(P.values, digits=3)
    write.csv(P.values, file=paste('KS_Test_',kk,'.csv'))
  }
